  ...
  return (
    <div className="mx-auto max-w-7xl space-y-6 p-4 lg:p-8">
      ...
      <div className="grid grid-cols-2 gap-3 sm:gap-4 lg:grid-cols-4">
        <div className="animate-stagger-up" style={{ animationDelay: "40ms" }}>
          <StatCard
            label="Total Task"
        ...
        <div className="animate-stagger-up" style={{ animationDelay: "100ms" }}>
          <StatCard
        ...
        <div className="animate-stagger-up" style={{ animationDelay: "220ms" }}>
          <StatCard
      ...
      <div className="animate-stagger-up rounded-2xl border border-border bg-card p-5 shadow-card" style={{ animationDelay: "260ms" }}>
        <div className="mb-3 flex items-center justify-between">
          <div>
            <h3 className="text-sm font-semibold text-foreground">Progress Workspace</h3>
            <p className="text-xs text-muted-foreground">
              Gabungan progress seluruh task di {activeWorkspace?.name}
            </p>
          </div>
          <span className="font-display text-3xl font-bold tabular-nums text-brand-600 dark:text-brand-400">{overallProgress}%</span>
        </div>
        <Progress value={overallProgress} size="md" />
        <div className="mt-4 grid grid-cols-2 gap-2.5 sm:grid-cols-4 sm:gap-3">
          {STATUS_ORDER.map((s) => (
            <div key={s} className="rounded-xl bg-muted px-3 py-2.5 text-center">
              <div className="font-display text-lg font-bold tabular-nums text-foreground">{byStatus[s]}</div>
      ...
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
        ...
        <div className="overflow-hidden rounded-2xl border border-border bg-card shadow-card">
          <div className="flex items-center justify-between border-b border-border px-5 py-4">
            <h3 className="text-sm font-semibold text-foreground">Task Terbaru</h3>
          </div>
                ...
                return (
                  ...
                  >
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <p className="text-xs text-muted-foreground">{project?.name} · {formatRelative(t.updatedAt)}</p>
                    </div>
              ...
              dueSoon.map((t) => {
                ...
                return (
                  <Link
                    ...
                    className="flex items-center gap-3 px-5 py-3 transition-colors hover:bg-muted"
                  >
                    <span className={`flex h-9 w-9 shrink-0 flex-col items-center justify-center rounded-lg text-center ${overdue ? "bg-rose-50 text-rose-600 dark:bg-rose-500/15 dark:text-rose-400" : "bg-muted text-muted-foreground"}`}>
                      <CalendarClock className="h-4 w-4" />
                    </span>
                    <div className="min-w-0 flex-1">
                      <p className="truncate text-sm font-medium text-foreground">{t.name}</p>
                      <p className={`text-xs ${overdue ? "font-medium text-rose-500 dark:text-rose-400" : "text-muted-foreground"}`}>
